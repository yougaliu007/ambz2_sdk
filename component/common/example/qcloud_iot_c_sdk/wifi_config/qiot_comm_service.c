    ...
    switch (cmd) {
        ...
        case CMD_SSID_PW_TOKEN: {
            ...
            if (ssid_json && psw_json && token_json) {
                ...
                ret = wifi_sta_connect(ssid_json->valuestring, psw_json->valuestring, 0);
                if (ret) {
...
int qiot_comm_service_start(void)
{
    ...
    int ret = xTaskCreate(_qiot_comm_service_task, COMM_SERVER_TASK_NAME, COMM_SERVER_TASK_STACK_BYTES, NULL,
