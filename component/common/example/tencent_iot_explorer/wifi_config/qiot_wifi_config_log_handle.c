...
int init_dev_log_queue(void)
{
    ...
    if (sg_dev_log_queue) {
        ...
        xQueueReset(sg_dev_log_queue);
        return 0;
    }

    sg_dev_log_queue = xQueueCreate(LOG_QUEUE_SIZE, LOG_ITEM_SIZE);
...
int push_dev_log(const char *func, const int line, const char *fmt, ...)
{
    ...
    uint32_t log_cnt = (uint32_t)uxQueueMessagesWaiting(sg_dev_log_queue);
    if (log_cnt == LOG_QUEUE_SIZE) {
        // pop the oldest one
        xQueueReceive(sg_dev_log_queue, log_buf, 0);
...
int app_send_dev_log(comm_peer_t *peer)
{
    ...
    do {
        memset(log_buf, 0, LOG_ITEM_SIZE);
        rc = xQueueReceive(sg_dev_log_queue, log_buf, 0);
        if (rc == pdPASS) {
...
int qiot_log_service_start(void)
{
    ...
    int ret = xTaskCreate(log_server_task, COMM_SERVER_TASK_NAME, COMM_SERVER_TASK_STACK_BYTES, NULL,
